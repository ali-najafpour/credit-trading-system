...
class Constraint
{
    ...
    {
        if ($this->operator == Constraint::OPERATOR_GROUP) {
            $values = $this->value;
            $method = $mode != static::MODE_OR ? 'where' : 'orWhere';
            $builder->$method(function ($query) use ($field, $values, $builder) {
                foreach ($values as $val) {
                    if (str_starts_with($val, '(g)')) {
                        preg_match('/^\((gt|g)\)(.+)$/', $val, $match);
                        $query->orWhere($field, '>', $match[2]);
                    } elseif (str_starts_with($val, '(ge)')) {
                        preg_match('/^\((ge)\)(.+)$/', $val, $match);
                        $query->orWhere($field, '>=', $match[2]);
                    } elseif (str_starts_with($val, '(l)')) {
                        preg_match('/^\((l)\)(.+)$/', $val, $match);
                        $query->orWhere($field, '<', $match[2]);
                    } elseif (str_starts_with($val, '(le)')) {
                        preg_match('/^\((le)\)(.+)$/', $val, $match);
                        $query->orWhere($field, '<=', $match[2]);
                    } elseif (preg_match('/^<.+,.+>$/', $val)) {
                        $val = preg_split('/,/', trim($val, "<>"));
                        $query->orWhereBetween($field, $val);
                    }
                }
            });
    ...
    protected static function parseOperatorAndValue($value, $is_negation)
    {
        ...
        if ($result = static::parseEqualsGroupOperator($value, $is_negation)) {
            return $result;
        }
     ...
     * @return string[]
     */
    protected static function parseEqualsGroupOperator($value, $is_negation)
    {
        if (preg_match('/^{.+__.+}$/', $value)) {
            $value = preg_split('/__/', trim($value, "{}"));
            return [$is_negation ? static::OPERATOR_GROUP : static::OPERATOR_GROUP, $value];
        }
        return false;
    }
}
